...
// NextBytesFrom reads the next n bytes from the given io.Reader and returns them along with the number of bytes read and any error encountered.
//
// Parameters:
// - reader: The io.Reader from which to read the bytes.
// - n: The number of bytes to read.
// Returns:
// - ret: The bytes read from the reader.
// - w: The number of bytes read from the reader.
// - err: An error if there was a problem reading the bytes.
func NextBytesFrom(reader io.Reader, n int) (ret []byte, w int, err error) {
	in := make([]byte, n)
	read := 0
	for read < n {
		r, rErr := reader.Read(in[read:])
		read += r
		if rErr != nil {
			err = rErr
			break
